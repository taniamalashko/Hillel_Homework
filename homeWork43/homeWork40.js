...
const productWrap = document.querySelector('.info');
...
const NAME_REQUIRED = "Будь ласка, введіть ПІБ";
const CITY_REQUIRED = "Будь ласка, оберіть своє місто";
const SHIPPING_REQUIRED = "Будь ласка, введіть номер відділення Нової пошти";
const PAYMENT_REQUIRED = "Будь ласка, оберіть спосіб оплати";
const QUANTITY_REQUIRED = "Будь ласка, введіть кількість одиниць товару";

function validateField(value, id, message) {
  const inputField = document.querySelector(`#${id}`);
  const smallElement = inputField.nextElementSibling;
...
function validatePayment(value, id, message) {
  const inputField = document.querySelector(`#${id}`);
  const smallElement = inputField.lastElementChild;
...
//Рендер зменшеної картки після відкриття форми
function renderLittleCard(prod) {
  const littleCard = document.createElement('div');
  const littleImg = document.createElement('div');
  const littleInfo = document.createElement('div');
  const littleProdName = document.createElement('h3');
  const littleProdPrice = document.createElement('span');

  littleCard.classList.add('little-card');
  littleImg.classList.add('little-image');
  littleInfo.classList.add('little-info');
  littleProdName.classList.add('little-name');
  littleProdPrice.classList.add('little-price');

  littleImg.innerHTML = `<img src="${prod.img}" alt="${prod.productName}">`;
  littleProdName.innerText = prod.productName;
  littleProdPrice.innerText = prod.price;

  littleInfo.appendChild(littleProdName);
  littleInfo.appendChild(littleProdPrice);
  littleCard.appendChild(littleImg);
  littleCard.appendChild(littleInfo);
...
function showForm(prod) {
  formContainer.classList.remove('display__none');

  productWrap.innerHTML = '';
  productWrap.appendChild(renderLittleCard(prod));

  //перевірка форми та відкриття модального вікна
  form.addEventListener('submit', function(event){
    ...
    isValid = isValid && validateField(name, 'name', NAME_REQUIRED);
    isValid = isValid && validateField(city, 'city', CITY_REQUIRED);
    isValid = isValid && validateField(shipping, 'shipping', SHIPPING_REQUIRED);
    isValid = isValid && validatePayment(payment, 'payment', PAYMENT_REQUIRED);
    isValid = isValid && validateField(quantity, 'quantity', QUANTITY_REQUIRED);

    if (isValid) {
      this.reset();
      showModal(formData, prod);
    }
  })
}

function hideForm() {
  formContainer.classList.add('display__none');
}

// //Зміна дисплею модального вікна
function showModal(formData, prod){
  modal.classList.toggle('display__none');

  document.querySelector('#modalProdName').innerText = prod.productName;
  document.querySelector('#modalProdImg').src = prod.img;
  document.querySelector('#modalProdImg').alt = prod.productName;
  document.querySelector('#modalPrice').innerText = prod.price;
  document.querySelector('#modalQuantity').innerText = formData.quantity;
  document.querySelector('#modalSum').innerText = parseInt(prod.price) * formData.quantity + ' грн';
  document.querySelector('#modalName').innerText = formData.name;
  document.querySelector('#modalCity').innerText = formData.city;
  document.querySelector('#modalShipping').innerText = formData.shipping;
  document.querySelector('#modalPayment').innerText = formData.payment;
}
